...
**调研结论**: 在 Claude Code / OpenClaw 生态中，给 AI 添加浏览器操作能力的三大主流方案是 **Playwright MCP**、**browser-use** 和 **Stagehand**。三者底层均基于 Playwright，但设计理念完全不同。
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ❶ Playwright MCP (Microsoft)          ⭐ 推荐首选                  │
│     Stars: 30K │ 语言: TS │ 成本: 最低                              │
│     一行命令安装: claude mcp add playwright npx @playwright/mcp     │
│     特点: 无障碍树(非截图)、41+工具、零LLM调用                       │
│     适合: 90%的日常浏览器操作                                        │
│                                                                     │
│  ❷ browser-use (Python)                ⭐ 复杂任务首选               │
│     Stars: 85K │ 语言: Python │ 成本: 中等                          │
│     安装: uvx browser-use[cli] --mcp                                │
│     特点: 自主Agent、8+Watchdog、ChatBrowserUse专用模型              │
│     适合: 多步骤自主决策任务                                         │
│                                                                     │
│  ❸ Stagehand (Browserbase)             ⭐ 生产重复任务首选           │
│     Stars: 22K │ 语言: TS │ 成本: 首次中等,重复接近零               │
│     安装: npm install @browserbasehq/stagehand                      │
│     特点: act/extract/observe/agent四层API、智能缓存、自愈           │
│     适合: 高频重复自动化 + 云端部署                                  │
│                                                                     │
├─────────────────────────────────────────────────────────────────────┤
│  快速决策:                                                          │
│  • 只选一个 → Playwright MCP                                        │
│  • 需要AI自主浏览 → + browser-use                                   │
│  • 生产级重复任务 → + Stagehand                                     │
│  • 预算极有限 → Playwright MCP (零额外LLM成本)                      │
│  • Python技术栈 → browser-use                                       │
│  • 企业安全合规 → browser-use (纵深防御) 或 Stagehand (SaaS托管)    │
└─────────────────────────────────────────────────────────────────────┘
```
...
|---|---|---|---|
| **综合评分** | 8.1/10 | **8.4/10** | 7.4/10 |
| **月下载量** | **7.37M** | 6.32M | 2.73M |
| **安装时间** | 1 分钟 | 2 分钟 | 3 分钟 |
| **每千次操作成本** | **~$0.50** | ~$5-55 | ~$1.50-31.50 |
| **单次操作延迟** | **100-500ms** | 2-5s | 200ms-5s |
| **安全等级** | 开发/测试 | **企业级** | SaaS 托管 |
| **容错等级** | 基础 | **最强** | 智能自愈 |

> 详细分析请阅读下方 13 章完整报告（1700+ 行）。
...
8. [选型指南](#8-选型指南)
9. [总结与建议](#9-总结与建议)
10. [源码级深度剖析 (V2)](#10-源码级深度剖析v2-迭代新增)
11. [实战快速上手指南 (V3)](#11-实战快速上手指南v3-迭代新增)
12. [安全模型、社区生态与容错机制 (V4)](#12-安全模型社区生态与容错机制v4-迭代新增)
13. [FAQ 踩坑指南与组合使用架构 (V5)](#13-faq-踩坑指南与组合使用架构v5-迭代新增)
...
> **本报告版本**: V6 (2026-03-31) — 最终版  
> **迭代历程**: V1 架构概览 → V2 源码剖析 → V3 实战上手 → V4 安全/生态/容错 → V5 FAQ/组合架构/中文适配 → V6 执行摘要/决策卡  
> **报告规模**: 1700+ 行、13 章 + 执行摘要、涵盖 7 个项目源码深度阅读  
> **GitHub**: https://github.com/Zey413/browser-use-comparison  
> **所有项目已 clone 到本地，可随时深入查阅源码。**
